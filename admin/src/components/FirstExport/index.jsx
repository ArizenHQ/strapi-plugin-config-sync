...
const FirstExport = () => {
  ...
  return (
    <div>
      <EmptyStateLayout
        content={formatMessage({ id: 'config-sync.FirstExport.Message', defaultMessage: 'Looks like this is your first time using config-sync for this project.' })}
        action={(
          <ConfirmModal
            type="export"
            onSubmit={() => dispatch(exportAllConfig([], toggleNotification, formatMessage, post, get))}
            trigger={(
              <Button>{formatMessage({ id: 'config-sync.FirstExport.Button', defaultMessage: 'Make the initial export' })}</Button>
            )}
          />
        )}
