...
const ConfigPage = () => {
  ...
  return (
    <Layouts.Content paddingBottom={8}>
      <GithubRepoForm value="" onChange={() => {}} />
      {appEnv === 'production' && (
        <Box paddingBottom={4}>
          <Alert variant="danger">
            <Typography variant="omega" fontWeight="bold">{formatMessage({ id: 'config-sync.envWarning.production.heading', defaultMessage: "You're in the production environment" })}</Typography><br />
            {formatMessage({ id: 'config-sync.envWarning.production.message_1', defaultMessage: 'Please be careful when syncing your config in production.' })}<br />
            {formatMessage({ id: 'config-sync.envWarning.production.message_2', defaultMessage: 'Make sure you are not overriding critical config changes on import.' })}
          </Alert>
        </Box>
      )}
      <ActionButtons />
